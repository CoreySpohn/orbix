...
def test_keplerian_orbit_parity_with_planets_internal_prop():
    """KeplerianOrbit.propagate matches orbix.Planets.prop_AU on equivalent inputs.
    ...
    # Planets.prop_AU delegates to self.orbit.propagate
    r_planets = planets.prop_AU(trig_solver, t_jd)
...
def test_planets_exposes_orbit_field():
    ...
    assert isinstance(planets.orbit, KeplerianOrbit)
...
def test_planets_propagation_matches_standalone_orbit():
    ...
    np.testing.assert_allclose(r_planets, r_orbit, rtol=1e-10, atol=1e-12)
