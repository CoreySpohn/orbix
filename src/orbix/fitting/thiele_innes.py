...
def _solve_abfg(X, Y, ra, dec, ra_err, dec_err, corr):
    ...
    #
    # Rather than forming the full 2N×2N matrix, we use the normal
    # equations directly: (D^T W D) β = D^T W d
...
def _abfg_to_elements(A_scaled, F_scaled, B_scaled, G_scaled, e):
    ...
    The TI constants relate to orbital elements as:
        ...
        F = a·√(1-e²) * (-cosW·sinw - sinW·cosw·cosi)
        G = a·√(1-e²) * (-sinW·sinw + cosW·cosw·cosi)

    From these we can recover (a, i, W, w) using the standard
    identities:
        A² + B² = a² * (cos²w + sin²w·cos²i) ≡ a² * α
        F² + G² = a²(1-e²) * (sin²w + cos²w·cos²i) ≡ a²(1-e²) * β
        AF + BG = a²√(1-e²) * (-cosw·sinw(1-cos²i)) ≡ a²√(1-e²) * γ
        AG - BF = a²√(1-e²) * cosi

    The last identity gives cos(i) directly (up to sign of a²√(1-e²)).
    ...
    # A² + B² + F² + G² = a²(1 + cos²i) + a²(1-e²)(1 + cos²i) ... nope
    # Better approach: use the two independent sums.
    #
    # p² = A² + B² = a²(cos²w + sin²w·cos²i)
    # q² = F² + G² = a²(1-e²)(sin²w + cos²w·cos²i)
    # And: AG - BF = a²√(1-e²)·cosi

    p2 = A_scaled**2 + B_scaled**2
    q2 = F_scaled**2 + G_scaled**2

    # a² from: p² + q²/(1-e²) = a²(1 + cos²i)
    # and: (AG-BF)² = a⁴(1-e²)·cos²i
    # So a⁴(1-e²)·cos²i = (AG-BF)²
    # And a²(1+cos²i) = p² + q²/(1-e²)
    #
    # Alternative direct formula:
    # a²(cos²w + sin²w·cos²i) = p²
    # a²(1-e²)(sin²w + cos²w·cos²i) = q²
    # Sum: a²[cos²w + sin²w·cos²i + (1-e²)(sin²w + cos²w·cos²i)]
    #    = a²[(cos²w + (1-e²)sin²w) + cos²i(sin²w + (1-e²)cos²w)]
    #    = a²[1 - e²sin²w + cos²i(1 - e²cos²w)]
    # This is messy. The standard approach uses:
    #
    # d = AG - BF = a²√(1-e²)·cosi
    # s = A² + B² + (F² + G²)/(1-e²)
    #   = a²(cos²w + sin²w·cos²i + sin²w + cos²w·cos²i)
    #   = a²(1 + cos²i)
    #
    # Then a⁴(1-e²)cos²i = d²
    # and  a²(1+cos²i) = s
    #
    # From these: let u = a²·cos²i  (unknown)
    # d² = (1-e²)·u·a²  →  a² = d²/((1-e²)·u)
    # s = a² + u + a² - a²  ... hmm, s = a²(1+cos²i) = a² + u
    # So a² = s - u
    # And d² = (1-e²)·u·(s-u)
    # This is a quadratic in u:
    #   (1-e²)·u² - (1-e²)·s·u + d² = 0
    #   u = [(1-e²)·s ± √((1-e²)²s² - 4(1-e²)d²)] / (2(1-e²))
    #   u = [s ± √(s² - 4d²/(1-e²))] / 2

    d = A_scaled * G_scaled - B_scaled * F_scaled  # = a²√(1-e²)·cosi / dist_pc²
    s = p2 + q2 / jnp.maximum(1.0 - e**2, 1e-30)  # = a²(1+cos²i) / dist_pc²

    # Quadratic for u = a²·cos²i / dist_pc²
    ...
    # Since cos²i ≤ 1, we need u ≤ a² = s - u, i.e., u ≤ s/2.
    # The minus root gives the smaller u → cos²i ≤ 1.
    u = (s - sqrt_disc) / 2.0
    ...
    # cos_i from d = a²√(1-e²)·cosi → cosi = d / (a²√(1-e²))
    ...
    # F and G carry an extra √(1-e²) factor that must be divided out
    # before forming the sums.

    # Scale F, G by 1/√(1-e²) to put them on the same footing as A, B
...
def _compute_chi2_and_ll(X, Y, A_s, F_s, B_s, G_s, ra, dec, ra_err, dec_err, corr):
    """Compute χ² and log-likelihood for a TI fit.
...
def thiele_innes_fit(astrom_data, T, e, tp, Ms, dist_pc):
    ...
    # n = 2π/T directly — no need to go through Kepler III
    n = 2.0 * jnp.pi / T
    ...
    # Compute chi² and log-likelihood
