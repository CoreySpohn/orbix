...
def predict_rv(times, T, Ms, Mp_sini, e, cos_w, sin_w, tp):
    ...
    The ``√(1-e²)`` factor in K cancels with the orbital velocity prefactor,
    ...
    # e-independent base velocity scale: K0 = K * sqrt(1-e²)
    # K = (2πG/T)^(1/3) * Mp_sini / Ms^(2/3) / sqrt(1-e²)
    # K0 = (2πG/T)^(1/3) * Mp_sini / Ms^(2/3)
    K0 = (two_pi_G / T) ** (1.0 / 3.0) * Mp_sini / Ms ** (2.0 / 3.0)

    # Singularity-free RV: cancellation of sqrt(1-e²) factors
...
def predict_photometry(times, a, e, cos_i, W, cos_w, sin_w, tp, Ms, Lambda, dist_pc):
    ...
    Args:
        ...
        tp: Time of periapsis passage (days). Scalar.
        Ms: Stellar mass (kg). Scalar.
        Lambda: Photometric area ``Ag * Rp²`` (AU²). Scalar.
        dist_pc: Distance to system (parsec). Scalar.
    ...
    # Sky-plane distance: sqrt(x² + y²), observer along z-axis
    sky_sep_AU = jnp.sqrt(r[0] ** 2 + r[1] ** 2)
    alpha_arcsec = sky_sep_AU / dist_pc

    # Phase angle: cos(β) = -z/|r| (z toward observer, so phase angle is
    ...
    # Contrast: Fp/Fs = Lambda * phase / r²
    # Lambda = Ag * Rp² (both in AU² when Rp is in AU)
