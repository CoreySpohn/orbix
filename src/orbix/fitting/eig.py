...
noise covariance.  This replaces the O(seconds) optimizer with an O(μs)
...
if TYPE_CHECKING:
    from orbix.fitting.laplace import LaplaceMixtureResult
...
def _predict_astrom_pure(
    ...
    cos_i = phys["cos_i"]
    W = phys["W"]
    w = phys["w_raw"]  # after transform: physical ω
    M0 = phys["M0"]
...
def _predict_sep_dmag_pure(
    ...
    ``Lambda = Ag * Rp²`` is provided as a known constant (the planet
...
def geometric_eig(
    ...
    Args:
        ...
        jacobian: Forward-model Jacobian ``∂y/∂z``.  Shape ``(n_obs, D)``
    ...
    **Imaging-aware mode** (optional): When ``Lambda`` and
    ...
    Args:
        ...
        obs_variance: Measurement variance per observable.  Scalar
            (isotropic RA/Dec noise in arcsec²) or ``(n_obs,)``
            for heteroscedastic noise.
        Ms: Stellar mass (kg).
        dist_pc: Distance (parsec).
        Lambda: Planet photometric area ``Ag * Rp²`` (AU²).  When
